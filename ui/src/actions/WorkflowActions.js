...
export const searchWorkflows = (query, search, hours, fullstr, start) => async (dispatch, getState) => {
  dispatch({
    type: 'GET_WORKFLOWS',
    search
  });

  if (fullstr && search && search.length > 0) {
    search = '"' + search + '"';
  }
  
  try {
    const url = `/api/wfe/${status}?q=${query}&h=${hours}&freeText=${search}&start=${start}`;
