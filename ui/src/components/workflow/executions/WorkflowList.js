...
class Workflow extends React.Component {
  ...
  handleRowSelect(row, isSelected, e) {
    ...
    if (isSelected) {
      ...
      var index =
        this.state.selectedWFEs.filter(storedRow => {
  ...
  bulkProcess() {
    let wfes = this.state.selectedWFEs.map(wfe => {
      return wfe.workflowId;
    });
  ...
  render() {
    ...
    const bulkSpin = this.state.bulkProcessInFlight ? (
      <i style={{ fontSize: '150%' }} className="fa fa-spinner fa-spin" />
    ) : (
      ''
    );
    const bulkSuccess = this.state.bulkProcessSuccess ? (
      <span style={{ fontSize: '150%', color: 'green' }}>Success!</span>
    ) : (
      ''
    );
    const bulkValidation =
      this.state.bulkValidationMessage !== '' ? (
        <span style={{ fontSize: '150%', color: 'red' }}>{this.state.bulkValidationMessage}</span>
      ) : (
        ''
      );
    const bulkError = this.state.bulkError ? (
      <span style={{ fontSize: '150%', color: 'red' }}>Error! Please remove invalid workflows and resubmit!</span>
    ) : (
      ''
    );
    ...
    return (
      <div className="ui-content">
        ...
        <Panel header="Bulk Processing  (click to expand)" collapsible>
          ...
          <Grid fluid={true}>
            <Row className="show-grid">
              <Col md={3}>
                <Input plaintext>
                  <span style={{ fontSize: '150%' }}>{this.state.selectedWFEs.length} </span>
                </Input>
                &nbsp;
                <i className="fa fa-angle-up fa-1x" />
                &nbsp;&nbsp;&nbsp;
              ...
              <Col md={1}>
                <Button
                  bsStyle="success"
                  onClick={this.bulkProcess}
                  value="Process"
                  className="btn"
                  disabled={this.state.bulkProcessInFlight}
                >
                  Process
                </Button>
                &nbsp;
        ...
        >
          <TableHeaderColumn dataField="workflowType" dataAlign="left" dataSort={true}>
            Workflow
          </TableHeaderColumn>
          <TableHeaderColumn dataField="workflowId" isKey={true} dataSort={true} dataFormat={linkMaker}>
            Workflow ID
          </TableHeaderColumn>
